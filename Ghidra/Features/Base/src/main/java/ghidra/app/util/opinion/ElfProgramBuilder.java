...
class ElfProgramBuilder extends MemorySectionResolver implements ElfLoadHelper {
	...
	private void adjustSegmentAndSectionFileAllocations(ByteProvider byteProvider,
		...
		monitor.initialize(segments.size() + sections.size());

		for (ElfSegment segment : segments) {
			monitor.checkCanceled();
			monitor.incrementProgress(1);
			long size = segment.getFileSize();
			if (segment.getType() == ElfSegmentConstants.PT_NULL ||
		...
		for (ElfSection section : sections) {
			...
			if (section.getType() == ElfSectionConstants.SHT_NULL ||
				section.getType() == ElfSectionConstants.SHT_NOBITS ||
	...
	private void processGNU(TaskMonitor monitor) throws CancelledException {
		...
		if (versionTableAddr == null) {
			List<ElfSection> sections = elf.getSections(e -> e.getType() == ElfSectionConstants.SHT_GNU_versym);
			if (sections.size() == 0) {
				return;
			}
			versionTableAddr = findLoadAddress(sections.get(0), 0);
		}
...
//            }
	...
	private void adjustReadOnlyMemoryRegions(TaskMonitor monitor) {
		...
		setReadOnlyMemory(elf.getSection(e -> e.getNameAsString().equals(".data.rel.ro")), null);

		for (ElfSegment roSegment : elf.getSegments(e -> e.getType() == ElfSegmentConstants.PT_GNU_RELRO)) {
			// TODO: (GP-2730) Identify read-only region which should be transformed
			setReadOnlyMemory(elf.getSegment(
				ElfSegment.isProgramLoadHeaderContaining(roSegment.getVirtualAddress())),
				null);
		}
	}
	...
	private void markupInterpreter(TaskMonitor monitor) throws CancelledException {
		...
		List<ElfSegment> interpProgramHeaders = elf.getSegments(e -> e.getType() == ElfSegmentConstants.PT_INTERP);
		if (interpProgramHeaders.size() != 0) {
			long offset = interpProgramHeaders.get(0).getFileOffset();
			if (offset == 0) {
				log("ELF PT_INTERP appears to have been stripped from binary");
				return;
			}
			interpStrAddr = findLoadAddress(interpProgramHeaders.get(0).getFileOffset(), 1);
		}

		if (interpStrAddr == null) {
			ElfSection interpSection = elf.getSection(e -> e.getNameAsString().equals(ElfSectionConstants.dot_interp));
	...
	private void processRelocations(TaskMonitor monitor) throws CancelledException {
		...
		for (ElfRelocationTable relocationTable : relocationTables) {
			...
			if (section instanceof ElfSection) {
			...
			ElfSection sectionToBeRelocated = relocationTable.getSectionToBeRelocated();
	...
	private void processRelocationTable(ElfRelocationTable relocationTable,
		...
		if (unableToApplyRelocs) {
			...
			String relocTableName = tableSectionHeader instanceof ElfSection
				? ((ElfSection) tableSectionHeader).getNameAsString()
				: "dynamic";
			ElfSection sectionToBeRelocated = relocationTable.getSectionToBeRelocated();
	...
	private void markupProgramHeaders(TaskMonitor monitor) {
		...
		try {
			...
			monitor.initialize(programHeaders.size());
			...
			for (int i = 0; i < programHeaders.size(); i++) {
				...
				if (programHeader.getType() == ElfSegmentConstants.PT_NULL) {
					continue;
				}
				if (programHeader.getFileOffset() == 0) {
					continue; // has been stripped
				}
				Address segmentAddr = findLoadAddress(programHeader, 0);
				if (segmentAddr != null) {
	...
	private void markupSectionHeaders(TaskMonitor monitor) {
		...
		try {
			...
			List<ElfSection> sections = elf.getSections();
			monitor.initialize(sections.size());
			for (int i = 0; i < sections.size(); i++) {
				...
				Data d = array.getComponent(i);
				String comment = section.getNameAsString();
				String type = section.getTypeAsString();
				if (type != null) {
				...
				if (section.getType() == ElfSectionConstants.SHT_SYMTAB_SHNDX) {
					markupSymbolSectionHeaderIndex(section);
				}
			}
		}
		catch (Exception e) {
	...
	private void markupSymbolSectionHeaderIndex(ElfSection section) {
		Address sectionAddr = findLoadAddress(section, 0);
		if (sectionAddr == null) {
			return;
		}
	...
	private List<ElfSymbolTable> getGnuDebugDataSymbolTables(TaskMonitor monitor) {
		ElfSection debugDataSection = elf.getSection(e -> e.getNameAsString().equals(".gnu_debugdata"));
	...
	private Address calculateSymbolAddress(ElfSymbol elfSymbol) {
		...
		List<ElfSection> elfSections = elf.getSections();
		...
		if (sectionIndex == ElfSectionConstants.SHN_UNDEF) { // Not section relative 0x0000 (e.g., no sections defined)
		...
		else if (Short.compareUnsigned(sectionIndex, ElfSectionConstants.SHN_LORESERVE) < 0 ||
			sectionIndex == ElfSectionConstants.SHN_XINDEX) {
			...
			if (sectionIndex == ElfSectionConstants.SHN_XINDEX) {
				uSectionIndex = elfSymbol.getExtendedSectionIndex();
				if (uSectionIndex == 0) {
			...
			if (uSectionIndex < elfSections.size()) {

				ElfSection symSection = elfSections.get(uSectionIndex);
		...
		else if (sectionIndex == ElfSectionConstants.SHN_ABS) { // Absolute value/address - 0xfff1
		...
		else if (sectionIndex == ElfSectionConstants.SHN_COMMON) { // Common symbols - 0xfff2 (
...
//	}
	...
	private void markupGnuBuildId(TaskMonitor monitor) {
		...
		ElfSection sh = elf.getSection(e -> e.getNameAsString().equals(".note.gnu.build-id"));
		...
		try {
			listing.createData(addr,
				new GnuBuildIdSection(program.getDataTypeManager(), sh.getFileSize()));
		}
		catch (Exception e) {
	...
	private void markupGnuDebugLink(TaskMonitor monitor) {
		...
		ElfSection sh = elf.getSection(e -> e.getNameAsString().equals(".gnu_debuglink"));
		Address addr = findLoadAddress(sh, 0);
		if (addr == null) {
			return;
		}
		try {
			listing.createData(addr,
				new GnuDebugLinkSection(program.getDataTypeManager(), sh.getFileSize()));
		}
		catch (Exception e) {
...
//					Long.toHexString(stringTable.getFileOffset()));
	 ...
	 */
	private AddressSpace getSegmentAddressSpace(ElfSegment elfProgramHeader) {
		if (elfProgramHeader.getType() != ElfSegmentConstants.PT_LOAD &&
	 ...
	 */
	private Address getSegmentLoadAddress(ElfSegment elfProgramHeader) {
	 ...
	 */
	private AddressSpace getSectionAddressSpace(ElfSection elfSectionHeader) {
	 ...
	 */
	private Address getSectionLoadAddress(ElfSection elfSectionHeader) {
...
// TODO: Watch out for negative long addresses !!!!!!
			...
			if (section instanceof ElfSegment) {
				ElfSegment programHeader = (ElfSegment) section;
// FIXME! Inconsistent in use of VirtualAddress which is generally an addressable word offset - not byte offset
				...
				if (programHeader.getType() != ElfSegmentConstants.PT_LOAD) {
					// Check for PT_LOAD segment which may contain requested segment
					ElfSegment loadHeader = elf.getSegment(
						ElfSegment.isProgramLoadHeaderContaining(offsetAddr));
					if (loadHeader != null) {
				...
				ElfSection sectionHeader = elf.getSection(
					ElfSection.isSectionLoadHeaderContaining(offsetAddr));
				if (sectionHeader != null) {
					return findLoadAddress(sectionHeader, offsetAddr - sectionHeader.getVirtualAddress());
				}
			}
			else if (section instanceof ElfSection) {
				ElfSection s = (ElfSection) section;
				if (s.isAlloc()) {
	...
	private Address findLoadAddress(long fileOffset, long headerSize) {
		...
		for (ElfSegment segment : elf.getSegments()) {
			...
			if (fileOffset >= startOffset && headerEndOffset <= endOffset) {
				...
				if (headerAddr != null && segment.getType() == ElfSegmentConstants.PT_LOAD) {
					return headerAddr;
				}
			}
		}
	...
	private void expandProgramHeaderBlocks(TaskMonitor monitor) throws CancelledException {
		...
		List<ElfSegment> elfProgramHeaders = elf.getSegments();

		monitor.setMessage("Exapanding Program Segments...");
		monitor.initialize(elfProgramHeaders.size());
		for (int i = 0; i < elfProgramHeaders.size(); ++i) {
			ElfSegment elfProgramHeader = elfProgramHeaders.get(i);
			monitor.checkCanceled();
			monitor.incrementProgress(1);
	...
	private void processProgramHeaders(TaskMonitor monitor) throws CancelledException {
		...
		for (int i = 0; i < elfProgramHeaders.size(); ++i) {
			...
			if (elfProgramHeader.getType() == ElfSegmentConstants.PT_NULL) {
				continue;
			}
			...
			if (elfProgramHeader.getType() != ElfSegmentConstants.PT_LOAD) {
				...
				if (elf.getSegment(
					ElfSegment.isProgramLoadHeaderContainingFileOffset(fileOffset)) != null) {
					continue;
				}
				ElfSection section = elf.getSection(
					ElfSection.isSectionContainingFileRange(fileOffset, elfProgramHeader.getFileSize()));
				if (section != null) {
					log("Skipping segment[" + i + ", " + elfProgramHeader.getDescription() +
	 ...
	 */
	private void processProgramHeader(ElfSegment elfProgramHeader, int segmentNumber)
...
// process-specific extension should control the outcome.
		...
		try {
			...
			if (loadSizeBytes != 0) {
				addInitializedMemorySection(elfProgramHeader, elfProgramHeader.getFileOffset(),
					...
					isFragmentationOK,
					elfProgramHeader.getType() == ElfSegmentConstants.PT_LOAD);
			}
	...
	private String getSegmentName(ElfSegment elfProgramHeader, int segmentNumber) {
		int headerType = elfProgramHeader.getType();
		if (headerType == ElfSegmentConstants.PT_NOTE) {
			return "_elfNote";
		}
	...
	private long computeRelocationStartAddress(AddressSpace space, long baseOffset,
		...
		AddressSpace defaultSpace = getDefaultAddressSpace();
		List<ElfSection> sections = elf.getSections();
		for (ElfSection elfSectionToLoad : sections) {
			monitor.checkCanceled();
	...
	private void processSectionHeaders(TaskMonitor monitor) throws CancelledException {
		...
		for (ElfSection elfSectionToLoad : sections) {
			...
			if (type != ElfSectionConstants.SHT_NULL &&
				...
				if (size <= 0 ||
					(type != ElfSectionConstants.SHT_NOBITS && size >= fileBytes.getSize())) {
					log("Skipping section [" + elfSectionToLoad.getNameAsString() +
	 ...
	 * @throws AddressOutOfBoundsException if an invalid memory address is encountered
	 */
	private void processSectionHeader(ElfSection elfSectionToLoad,
		...
		if (sectionByteLength == 0 &&
			elfSectionToLoad.getType() == ElfSectionConstants.SHT_PROGBITS) {
			// Check for and consume uninitialized portion of PT_LOAD segment if possible
			ElfSegment loadHeader = elf.getSegment(
				ElfSegment.isProgramLoadHeaderContaining(addr));
			if (loadHeader != null) {
		...
		try {
			if (loadOffset == -1 ||
				elfSectionToLoad.getType() == ElfSectionConstants.SHT_NOBITS) {
	 ...
	 * @param loadable Corresponding ElfSection or ElfProgramHeader for the memory block to be created.
